...
def kabraji_ka_totka():
 # totka hai dosto !
 n,x,y,d=L() 
 def distance_between_2_pages(x,y):
  return (abs(x-y) + (d-1))//d;
 len_ = abs(x-y);
 res=imax;
 if(len_%d == 0):
  res=min(res,distance_between_2_pages(x,y))
 len_ = y-1;
 if len_%d==0:
  res=min(res,distance_between_2_pages(x,1) + distance_between_2_pages(1,y))
 len_ = n-y;
 if len_ %d==0:
  res=min(res,distance_between_2_pages(x,n) + distance_between_2_pages(n,y))
 print(-1 if res==imax else res)
