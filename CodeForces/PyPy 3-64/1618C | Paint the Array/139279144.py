...
def main():
    ...
    even = arr[1]
    for i in range(2,n):
        if (i+1) % 2 == 1:odd = gcd(odd, arr[i])
        else:even = gcd(even, arr[i])
    foundA=1;
    foundB = 1
    for i in range(n):
        if(i+1)%2==0:
            if(arr[i]%odd == 0):foundA = 0;
        else:
            if arr[i] % even == 0:foundB = 0;
 
    if not foundA and not foundB:print(0)
    elif foundA:print(odd)
    else:print(even)
#-------------------------------------------------------------------------------------------------------------------------------------
BUFSIZE = 8192
class FastIO(IOBase):
...
if __name__ == "__main__":
    maths = maths()
    IO()
    dsu = lambda x : DisjointSetUnion(x)
    if test_case_run__:
        for i in range(I()):
