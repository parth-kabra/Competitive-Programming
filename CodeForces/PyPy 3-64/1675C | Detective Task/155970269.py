...
def main():
    s = inp()
    n = len(s)
    if '1' not in s and '0' not in s:print(n);return
    ...
    for i in range(n):
        ...
        if s[i] == '0':
            res += 1;i -= 1
            while i >= 0 and s[i] == '?':res += 1;i -= 1
            if i >= 0 and s[i] == '1':res += 1
            break;
    if res:
        print(res);return
