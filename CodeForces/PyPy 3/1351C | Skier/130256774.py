...
def palin(s):return s==reversed(s)
...
def kabraji_ka_totka():
 ...
 for reached in range(n):
  temp_standing = initial_standing[:]
  if(s[reached] == "N"):
   initial_standing[0]+=1
  elif (s[reached] == "S"):
   initial_standing[0]-=1
 
  elif s[reached] == "E":
   initial_standing[1]+=1
  else:
   initial_standing[1]-=1
 
  if tuple(temp_standing + initial_standing) in visited_points or tuple(initial_standing + temp_standing) in visited_points:
   res+=1
  else:
   visited_points.add(tuple(temp_standing+initial_standing))
   res+=5
 print(res)
