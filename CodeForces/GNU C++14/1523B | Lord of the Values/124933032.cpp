...
vector<tuple<int,int,int>> helper(vector<int> &arr,int n,vector<int> &tofind){
    vector<tuple<int,int,int>> result;
    for(int i=1;i<n;i+=2){
        for(int &j : tofind){
            result.push_back({j,i,i+1});
        }
    }
    return result;
}
void kabraji_ka_totka(){
    ...
    while(t--){
        ...
        vector<int> tofind={1,2,1,1,2,1};
        vector<tuple<int,int,int>> result=helper(arr,n,tofind);
