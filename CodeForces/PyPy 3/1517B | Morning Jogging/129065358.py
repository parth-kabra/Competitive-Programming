...
def kabraji_ka_totka():
 ...
 """brr=[]
    ...
    for i in vec:
     g=s[]"""
 arr=[L() for i in range(n)];
 runners=[[[],imax] for i in range(m)] #for i in range(m)
 for i in range(n):
  runners.sort(key=lambda x : x[1])
  arr[i].sort(reverse=true)
  for j in range(m):
   runners[j][0].append(arr[i][j])
   runners[j][1]=min(runners[j][1],arr[i][j])
 for i in range(n):
  print(" ".join(str(runners[j][0][i]) for j in range(m)))
