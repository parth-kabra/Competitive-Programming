...
def main():
    ...
    mp = Counter(arr);
    max_freq = 1;
    max_freq_element = max(arr)
    for item, value in mp.items():
        if value > max_freq:
            max_freq = value;
            max_freq_element = item;
    #print(max_freq,max_freq_element)
    """while max_freq_element in arr:
        arr.remove(max_freq_element)
    """
    arr = remove_items(arr, max_freq_element)
    print(len(arr))
