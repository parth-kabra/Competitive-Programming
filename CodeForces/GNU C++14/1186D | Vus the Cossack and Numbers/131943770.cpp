...
void seive(){memset(prime,true,sizeof(prime));prime[0] = prime[1] = false;for(int i = 2; i < N; i++){if(prime[i]){for(int mask = (i<<2); mask < N; mask++){prime[mask] = false;}}}}
...
void solve(){
    ...
    for(double &i : arr){
        cin>>i;
        int found_low = (i < 0) ? ceil(i) : floor(i);
        int found_high = round(i);
        lower.push_back(found_low);
        higher.push_back(found_high);
 
        low += found_low;
        high += found_high;
    }
    debug(make_pair(lower,low));
    debug(make_pair(higher,high));
